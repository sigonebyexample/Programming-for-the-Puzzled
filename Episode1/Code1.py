...
#the above algorithm is clever but a bit complex
#here is a simpler algorithm that is less efficient but works fine
#for the input sizes we care about
def best_time_sweep(schedule):
    ...
    # Check each time to find maximum overlap
    max_count = 0
    best_time = 0
...
# Test
result = best_time_sweep(sched)
print(f'Best time: {result[0]} with {result[1]} celebrities')
